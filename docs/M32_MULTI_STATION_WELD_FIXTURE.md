...
recorded in the manufacturing frame with source-vector evidence. Product
bounds and every review instance use the accepted full source-to-manufacturing
transform; fixture AABBs, OCP solids, process sweeps, and pitch checks remain in
that manufacturing frame. Manufacturing-axis labels are never compared
directly with source geometry, and source STEP coordinates remain unchanged.
...
confirmed evidence. Each complete joint position is transformed into the
manufacturing frame and evaluated independently against authored fixture components, closed clamp
