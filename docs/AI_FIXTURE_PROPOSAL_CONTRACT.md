...
`AiFixtureProvider` is provider-neutral. The default live adapter is the
OpenAI Responses API when both of these process-environment values are present:

- `OPENAI_API_KEY`
- `FXD_OPENAI_MODEL` (or the compatibility name `FXD_AI_MODEL`)

`FXD_AI_PROVIDER=openai` explicitly selects it. FXD does not choose, infer, or
persist a model name. The adapter makes one bounded `POST` to the official
Responses endpoint, uses `text.format` JSON Schema with `strict: true`, sends
no raw STEP bytes, limits compact context and response sizes, and makes no
...
Timeout, refused or incomplete output, provider failure, or quarantined output produces
