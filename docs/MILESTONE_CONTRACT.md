...
The Foreman must verify that the registry-selected issue number resolves in `kool1160/fxd-fixture-design`, that the returned number still matches the selection, and that its state is exactly `OPEN` before appending the issue body, invoking Codex or another paid provider, creating a branch, or generating a patch. A fetch failure, repository or number mismatch, closed issue, or unavailable body fails the run before implementation work.
...
Closeout uses two offline-verifiable phases because a pull request cannot know its own future merge commit. First, a closeout evidence PR records its known PR number, every implementation PR and its distinct PR-number-bearing merge evidence, reviewable results for every selected evidence profile, explicit human approvals, risks, and decisions while the milestone remains `Active`. After that PR is approved and merged, a distinct state-finalization PR records its own PR number and the now-existing closeout merge SHA, changes the milestone to `Complete`, and applies the approved next-lane disposition. Before and after merge, the finalization PR is proved only by its exact locally fetched GitHub `refs/pull/<PR>/head` state carrying the same registry blob that is present in current history. `refs/remotes/origin/pull/<PR>/head`, other remote-tracking refs, branches, tags, symbolic refs, alternate namespaces, PR numbers, and PR-shaped commit subjects are not evidence. The dedicated ref remains distinct from implementation and closeout evidence and supports ordinary merge and squash outcomes without trusting arbitrary commit text. Implementation and closeout merge commit subjects must retain GitHub's PR-number form so offline validation can prove their declared PR-to-commit associations. One commit or pull ref cannot supply evidence for more than one completion stage. No phase may be combined with another.

The finalization pull-head tree is compared directly with the reviewed closeout merge tree. It may modify only `docs/MILESTONE_STATE.json` and, when only an existing current-status projection is reconciled in place, `README.md`, `BACKLOG.md`, `docs/ROADMAP_QUEUE.md`, `docs/STRATEGY_HANDOFF.md`, `docs/LOCAL_ENGINEERING_WORKBENCH.md`, or `docs/project-records/README.md`. It may not add, delete, or rename files; modify workflows, dependencies, product code, tests, arbitrary prose, historical snapshots, or binary content; or carry any implementation or unrelated repository change. The validator centralizes both this exact path allowlist and the content-limited derived-status rule and reports every unauthorized operation and path.
...
9. a distinct state-finalization pull request records its own PR number, verifies that only the exact locally fetched, direct, nonsymbolic `refs/pull/<PR>/head` carries the proposed registry both before and after merge, verifies that its tree differs from the reviewed closeout tree only by the centralized finalization path/content allowlist, verifies that the same registry state is present in current history, verifies that the closeout merge commit's registry blob contains the sole `Active` milestone, its declared closeout PR, its implementation PR merge evidence, and complete results for every selected evidence profile; then records that already-present closeout merge commit, verifies its PR-number association from local Git history, and applies the `Complete` registry state; arbitrary commit subjects and alternate ref namespaces are not finalization evidence; and
10. the registry contains reviewable evidence entries for every selected evidence profile.

The implementation PR, closeout evidence PR, and state-finalization PR must remain separate for milestones governed by this contract. The final closeout decision must link the closeout PR number and its locally present merge commit. The complete closeout registry is compared as structured data with the finalization registry. State finalization may add only its distinct PR number, the closeout merge SHA, that SHA appended to reviewed merge evidence, the `Complete` status, exactly one closeout PR/SHA linkage decision, the already-approved next-lane disposition, the exactly one-step sequence revision required by that disposition, and at least one genuinely new approving sequence decision. It must not change any other root field, maintenance record, milestone, evidence, sequence identity, historical reconciliation, replacement relationship, or reviewed closeout value. State finalization changes governance state; it does not conceal implementation or evidence changes.

## Sequence changes and interruptions

Changing order, inserting, superseding, cancelling, replacing, pausing, reopening, or activating a milestone requires an explicit human-approved decision recorded in `sequence_decisions`, an exactly one-step increment to `sequence_revision`, valid predecessor relationships, and updated issue links. Every transition between legal statuses is represented explicitly rather than collapsing closed and planned states. Reopening `Complete`, `Superseded`, or `Cancelled` work into `Planned`, `Active`, `Blocked`, `Waiting`, or `Paused` requires a new approving decision that identifies the reopened milestone and authority while preserving prior implementation, completion, closeout, replacement, reconciliation, and decision history. Transitions into or out of `Paused`, changes to a formally paused lane's reason, decision, or active projection, and reopening a paused lane are sequence changes. Evidence-only changes that do not alter sequence, status, predecessor, replacement, or product-lane authority do not require a sequence revision. Each revision preserves prior decisions in order and appends at least one genuinely new approving decision; reordering, removing, duplicating, or merely reusing prior decisions does not authorize a new revision. The offline validator compares a branch registry with its `origin/main` merge base, falling back on main to the latest registry-changing commit's parent, so a later commit in the same PR cannot hide an omitted revision bump. A `Superseded` record names its replacement or explicit disposition. A `Cancelled` record names the cancellation decision. Milestones at or after `governance_effective_milestone` cannot opt into legacy reconciliation. Silent renumbering and milestone skipping are prohibited.
