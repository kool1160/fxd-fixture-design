...
class M32VisualReviewBundleTests(unittest.TestCase):
    def test_bundle_persists_exact_reloadable_governed_scenario_outside_repository(self):
        with tempfile.TemporaryDirectory() as parent:
            ...
            self.assertEqual(report["authored_geometry"]["local_station_plate_count"], 4)
            self.assertEqual(report["authored_geometry"]["provisional_closed_clamp_count"], 4)
            self.assertEqual(report["authored_geometry"]["provisional_open_clamp_envelope_count"], 4)
